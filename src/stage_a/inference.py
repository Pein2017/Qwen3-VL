...
def _trim_trailing_eos_pad(gen_ids: torch.Tensor, eos_id: Optional[int], pad_id: Optional[int]) -> torch.Tensor:
...
def discover_groups(
    root: Path, mission: Optional[str] = None
) -> Dict[str, GroupInfo]:
    ...
    if mission:
        ...
        if not mission_dirs[0].exists():
            raise FileNotFoundError(
                f"Mission directory not found: {mission_dirs[0]}"
            )
    else:
...
def infer_one_image(
    ...
    try:
        ...
        if grid is not None:
            ...
            logger.debug(
                ...
                tuple(grid[0].tolist()), expected_tokens, text_token_count,
            )
    except Exception:
        pass
    ...
    if not clean_text:
        raise ValueError("Empty summary generated (clean_text is empty after stripping)")
...
def process_group(
    ...
    for idx in range(1, num_images + 1):
        ...
        if key not in per_image:
            raise ValueError(
                f"Missing 图片_{idx} in per_image for group {group_id}"
            )
    ...
    record = {
        ...
        "images": [str(p.resolve()) for p in group_info.paths],
        "per_image": per_image,
