...
def _render_guidance_snippets(experiences: Dict[str, str]) -> str:
    ...
    Args:
        experiences: Dict mapping experience keys (e.g., "G0", "G1") to experience text

    Returns:
        Formatted text block with numbered experiences (e.g., "[G0]. ...\n[G1]. ...")

    Raises:
        ValueError: If experiences dict is empty
    """
    if not experiences:
...
def build_user_prompt(ticket: GroupTicket, guidance: MissionGuidance) -> str:
    ...
    Args:
        ticket: Group ticket with Stage-A summaries
        guidance: Mission guidance with experiences dict

    Returns:
        Formatted user prompt with experiences prepended

    Raises:
        ValueError: If experiences dict is empty (no empty fallback)
    """
