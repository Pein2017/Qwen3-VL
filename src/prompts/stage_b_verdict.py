...
【输入说明】
- 非无关图片摘要为单行 JSON 字符串（含 dataset/objects_total/统计，可选 备注/分组统计）；出现“无关图片”则视为与本任务无关。
...
- 通用负项触发词（仅动词/形容词/短语，不含名词）：
  未按要求、错误、缺失、松动、损坏、方向不正确、反向、不符合、不合格、不合理、未安装、未配备
- Pattern-first：若摘要中出现 `符合性` 的 `不符合` 或 `问题` 里的具体故障（例如露铜/未拧紧/生锈），均视为明确负项，但仍仅对与当前任务要点相关的内容执行 fail-first。
"""

_SOFT_SIGNALS_SYSTEM_PROMPT = """【软信号：备注 + 待确认信号】
- 多图摘要可能包含 `备注` 列表：备注是补充信息，判定以备注具体内容与多图证据为准。
- "无法确认/无法判断/可见性=部分/模糊"等属于待确认信号：本身不是明确负项，但也不能忽略。
- 输入中包含 `ImageN(obj=...)`（来自摘要 JSON 的 objects_total），用于了解图片复杂度。
- 若任务要点在所有图片中都无法"明确确认"，判不通过；若同一要点多图矛盾，优先用可见性=完整的证据消解，无法消解则判不通过。
...
- “无关图片”视为与本任务无关的图片：不得参与任何安装点/配对推断，也不得据此判不通过或判通过。
- 安装点/配对的默认口径：跨图只按“站点距离=数字”合并。
- `备注` 仅可用于判不通过（当备注包含与本任务要点直接相关的明确负项/缺失）；不得作为判通过依据。
- RRU安装检查/RRU位置检查/RRU线缆 三个任务相互独立：不得把其他任务的负项当作本任务判定依据，只围绕本任务检查要点判断。
"""

_BBU_COMMON_SYSTEM_PROMPT = """【BBU任务通用补充提示（默认规则，若结构不变量/补充提示更严格，以其为准）】
- “无关图片”视为与本任务无关：不得作为通过/不通过依据，也不得参与任何缺失推断。
