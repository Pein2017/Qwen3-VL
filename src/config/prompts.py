...
SYSTEM_PROMPT_SUMMARY = (
    ...
    "输出要求：\n"
    '1) 顶层按图片分组：{"图片_1": "...", "图片_2": "..."}；按出现顺序从 1 开始。\n'
    "2) 每个图片的值是一行摘要（纯文本字符串），覆盖该图片中检测到的所有相关对象。\n"
    "3) 相同对象必须合并计数，使用“对象描述×N”（×为全角乘号）；各条目之间仅用斜杠'/'分隔，并按“自上到下、再从左到右”的出现顺序。\n"
    '4) 对象描述沿用密集标注的层级规范：类型/属性[,属性]/[条件属性]；必要时在末尾追加"备注:..."。\n'
    "5) 将任何逗号替换为斜杠'/'，统一使用斜杠作为唯一分隔符。\n"
...
USER_PROMPT_SUMMARY = (
    ...
    "按图片顺序（图片_1、图片_2...）分组；条目之间仅用'/'分隔，并按“自上到下、再从左到右”排序。"
