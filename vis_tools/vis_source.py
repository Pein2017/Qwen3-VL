...
from src.datasets.geometry import points_to_xyxy  # noqa: E402
...
# Configuration
# ============================================================================
input_path = "public_data/lvis/resized_32_768/train.jsonl"  # Path to JSONL (e.g., data/bbu_full_768/all_samples.jsonl)
output_dir = "vis_out/lvis"  # Directory to save visualizations
...
filter_image_name = None  # Set to None to disable filtering
max_poly_points = 12  # Max polygon points to visualize (None = no limit, polygons exceeding this will be converted to bbox_2d)
...
def to_vis_objects(
    ...
    color_by: str,
    max_poly_points: Optional[int] = None,
) -> List[Dict[str, Any]]:
    ...
    for obj in sample_objects or []:
        if "bbox_2d" in obj:
            ...
            if max_poly_points is not None and pts:
                point_count = len(pts) // 2
                if point_count > max_poly_points:
                    # Convert polygon to axis-aligned bounding box using centralized utility
                    pts = points_to_xyxy(pts)
                    gtype = "bbox_2d"
        elif "line" in obj:
            gtype = "line"
            pts = obj["line"]
...
def visualize(
    ...
    for idx, rec in enumerate(
        ...
        obj_list = to_vis_objects(
            rec.get("objects", []), color_by=color_by, max_poly_points=max_poly_points
        )
