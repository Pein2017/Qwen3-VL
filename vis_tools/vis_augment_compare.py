...
def visualize_samples(cfg: VisConfig) -> None:
    ...
    use_yaml = False
    yaml_pipeline: Compose | None = None
    yaml_label: str = ""
    if cfg.config_yaml:
        try:
    ...
    for idx, rec in enumerate(records):
        ...
        for j in range(int(cfg.variants)):
            ...
            if use_yaml and yaml_pipeline is not None:
...
if __name__ == "__main__":
    ...
    if cfg.config_yaml:
        ...
        print(
            f"  ✓ Geometric: hflip({cfg.hflip_p:.0%}), vflip({cfg.vflip_p:.0%}), rot(±{cfg.max_deg}°), scale({cfg.scale_lo}-{cfg.scale_hi})"
        )
        print(f"  ✓ Resize: {cfg.resize_lo}x - {cfg.resize_hi}x resolution changes")
        print(
            f"  ✓ Crop: {cfg.random_crop_p:.0%} prob, min_coverage={cfg.crop_min_coverage}, min_objects={cfg.crop_min_objects}"
        )
        print(f"  ✓ Color: brightness/contrast/saturation {cfg.color_brightness}")
        print(
            f"  ✓ Advanced: gamma({cfg.gamma_p:.0%}), hsv({cfg.hsv_p:.0%}), clahe({cfg.clahe_p:.0%}), sharpness({cfg.sharpness_p:.0%})"
        )
        print(
            f"  ✓ Effects: solarize({cfg.solarize_p:.0%}), posterize({cfg.posterize_p:.0%}), autoContrast({cfg.auto_contrast_p:.0%})"
        )
    print("=" * 70)
