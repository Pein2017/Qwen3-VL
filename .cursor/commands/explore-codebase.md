...
description: Explore the broader codebase depth-first around a specific topic before proposing answers or edits.
---
$ARGUMENTS
...
2) Load governing instructions (e.g., `openspec/AGENTS.md` if relevant).  
...
5) Capture concise notes for summary/output; do not edit existing files.

**Exploration Phases (depth-first)**
1) Documentation: find topic-relevant READMEs, specs, ADRs; map stated contracts and workflows.  
2) Entry Points: identify scripts/CLIs/mains that invoke the topic across the workspace; note args/config knobs.  
...
- Topic summary: what the topic is and how it flows through the system (including relevant packages and services).  
