...
# Single-process, model-parallel sharding (HF device_map) across multiple GPUs.
# Useful when the model does not fit on a single GPU and you can trade throughput for memory.
stage_b_mode=model_parallel gpus=0,1,2,3 bash scripts/stage_b.sh
...
- When `gpus` contains multiple devices and `stage_b_mode=auto` (default), `scripts/stage_b.sh` auto-launches single-node `torchrun`; Stage-B runs **ticket-parallel rollout** across ranks while keeping rule-search proposing/gating on rank 0.
- In torchrun ticket-parallel mode, the model is replicated per rank (data-parallel); `model.device_map` is overridden to force single-GPU placement per rank (avoid accidental model-parallel sharding).
- When `stage_b_mode=model_parallel`, Stage-B runs single-process Python and relies on HF `device_map` sharding across GPUs (slower but can fit larger checkpoints).
...
- Stage‑B generation (prompt rendering + decode) is centralized in `src/generation` (Stage‑B runtime uses the HF backend today; a vLLM colocate backend exists in the engine but is not wired into `scripts/stage_b.sh` yet) to keep behavior consistent with Stage‑A and `vis_tools`.
  - The engine returns `text` (clean, special tokens removed) and `raw_text` (includes special tokens for debugging).
  - Stop-policy is standardized across the repo: only Qwen stop tokens are allowed (`<|endoftext|>`, `<|im_end|>`). Any other stop strings are invalid and will raise during config load.
  - If you manually use the vLLM backend: it only supports simple post-decode cleanup (`postprocess_text`). Plugins requiring HF-only hooks (e.g., `StoppingCriteria`/`LogitsProcessor`) fail fast on vLLM.
...
- `domain_map` / `default_domain`: Mission → domain mapping for runtime domain packs (`bbu|rru`). Stage‑A summary prompts are domain-aware (`bbu` vs `rru`), and Stage‑B system prompts are also domain-aware; Stage‑B’s mission-level “what to check” focus is carried by the mission guidance (notably `G0`) and the `任务/重点` fields in the user prompt.
- `stage_b_distillation`: optional post-convergence ChatML export (`distill_size`, `distill_seed`, `distill_temperature`).
- `sampler.grid.stop`: stop tokens for generation MUST be exactly `["<|endoftext|>", "<|im_end|>"]` (or omitted to use defaults). Any other stop strings are invalid and will raise.
...
- Stage-A 摘要中的“部分/无法判断”等软信号仅作为提示，不作为硬触发词（第三态已移除）。
- 推理 user prompt 会提供每张图的 `ImageN(obj=...)` 统计（由摘要 `统计` 推导的对象数量估计），用于帮助模型推断全局图/局部特写图并进行多图协同判断。
