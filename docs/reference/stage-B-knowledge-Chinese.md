...
- RRU安装检查：RRU设备 与 紧固件 匹配（同一安装点合并口径见 guidance）
- RRU位置检查：RRU接地端 + 接地线（有标签）+ 标签文本可读；标签内容不作强制限制
- RRU线缆：尾纤 + 套管保护正项 + 标签文本可读；标签内容不作强制限制

RRU 三个任务相互独立：不得用其它任务的负项影响当前任务判定，仅围绕本任务要点判断。
...
- 全局图通常对应 Stage‑A 摘要中识别到的物体数量更多（由 `统计` 推导的 `obj` 估计得出）；Stage‑B user prompt 会显式提供 `ImageN(obj=...)` 统计，帮助推断全局候选。
