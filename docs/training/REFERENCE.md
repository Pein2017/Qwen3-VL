...
- **Centralized generation engine**: Stage-A/Stage-B/`vis_tools` now load and generate via `src/generation` (HF backend today; a vLLM colocate backend exists in the engine but is not wired into the default Stage-A/B entrypoints). Chat-template rendering always disables “thinking” (`enable_thinking=False`) with a compatibility fallback; stop-policy handling is normalized with deterministic post-truncation and Stage‑B rollout stop strings are standardized to Qwen stop tokens only (`<|endoftext|>`, `<|im_end|>`). The engine returns `text` (clean, special tokens removed) and `raw_text` (includes special tokens for debugging). VLM preprocessing (left padding + pixel budget) is centralized in the engine to preserve Stage-A behavior.
