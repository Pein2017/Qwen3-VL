...
def test_polygon_conversion():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if "segmentation" in ann and isinstance(ann["segmentation"], list):
                    ...
                    for part in ann["segmentation"]:
                        if isinstance(part, list):
                            ...
                            # Test conversion
                                result = converter._extract_quads(ann, "test_category", image_info)
                                if result:
                                    print(f"    ✓ Converted {num_points}-point polygon → poly")
                                break
        ...
        for key in sorted(polygon_stats["point_distribution"].keys(), 
...
def test_qwen3vl_format_compliance():
    ...
    sample_polygon = {
        ...
        "objects": [
            {"poly": [100.0, 200.0, 300.0, 200.0, 300.0, 400.0, 100.0, 400.0], 
             "poly_points": 4, "desc": "sign"},
            {"poly": [50.0, 50.0, 60.0, 55.0, 70.0, 60.0, 80.0, 50.0, 75.0, 40.0, 65.0, 35.0],
             "poly_points": 6, "desc": "polygon"}
        ],
        "width": 640,
        "height": 480
    }
    ...
    def validate_sample(sample, name):
        ...
        if not isinstance(sample.get("images"), list):
            ...
            errors.append(f"'images' should have 1 element, got {len(sample['images'])}")
        ...
        for i, obj in enumerate(sample.get("objects", [])):
            ...
            if len(geom_fields) != 1:
                errors.append(f"Object {i}: need exactly 1 geometry field, got {geom_fields}")
