...
class JSONLValidator:
        ...
        with open(jsonl_path, "r", encoding="utf-8") as f:
            for line_num, line in enumerate(f, 1):
                ...
                try:
                    ...
                    self.errors.append(
                        ValidationError(line_num, "json", f"Invalid JSON: {e}")
                    )
                    continue
    ...
    def validate_sample(
        self, sample: Dict[str, Any], line_num: int, base_dir: str
    ) -> bool:
        """
        Validate one sample.
        ...
        for field in required_fields:
            if field not in sample:
                self.errors.append(
                    ValidationError(line_num, field, "Required field missing")
                )
                sample_valid = False
    ...
    def validate_object(
        ...
        if "bbox_2d" not in obj:
            self.errors.append(
                ValidationError(line_num, f"{prefix}.bbox_2d", "Required field missing")
            )
            return False
...
Examples:
  ...
  python validate_jsonl.py lvis/processed/train.jsonl --verbose
    ...
    validator = JSONLValidator(
        check_images=not args.skip_image_check, verbose=args.verbose
    )
