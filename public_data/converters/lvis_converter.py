...
class LVISConverter(BaseConverter):
    ...
    def _build_category_map(self, categories: List[Dict]) -> None:
        ...
        print(f"    Polygon mode: {'enabled (N-point polygons → quad)' if self.use_polygon else 'disabled (bbox only)'}")
    ...
    def _convert_annotation(
        ...
        width = x2 - x1
        height = y2 - y1
        if width < self.config.min_box_dimension or height < self.config.min_box_dimension:
            self.stats["skipped_objects"] += 1
            return None
    ...
    def _extract_quads(
        ...
        for part in segmentation:
            ...
            try:
                ...
                if (min(xs) < -10 or max(xs) > image_info["width"] + 10 or
                    min(ys) < -10 or max(ys) > image_info["height"] + 10):
