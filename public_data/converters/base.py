    ...
    def __post_init__(self):
        ...
        for field_name in ["input_path", "output_path", "image_root"]:
            ...
            if not os.path.isabs(path):
                raise ValueError(
                    f"{field_name} must be absolute path, got: {path}"
                )
        ...
        if not os.path.exists(self.image_root):
            raise ValueError(
                f"image_root does not exist: {self.image_root}"
            )
        ...
        if self.min_box_dimension < 0:
            raise ValueError(f"min_box_dimension must be >= 0, got {self.min_box_dimension}")
...
class BaseConverter(ABC):
    ...
    def _print_summary(self) -> None:
        ...
        print(f"  Avg objects/image: {self.stats['total_objects'] / max(self.stats['total_images'], 1):.2f}")
        print("="*60 + "\n")
