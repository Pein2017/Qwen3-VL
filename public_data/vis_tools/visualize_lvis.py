...
def find_available_images(image_root, image_ids, max_check=100):
    ...
    for img_id in sample_ids:
        ...
        file_name = (
            img_info.get("coco_file_name")
            or img_info.get("file_name")
            or img_info.get("coco_url", "").split("/")[-1]  # Extract from URL
        )
...
def visualize_sample(
                    ...
                    if isinstance(seg, list) and len(seg) >= 6:
                        ...
                        if show_labels:
                            ...
                            ax.text(
                                ...
                                ha="center",
                                bbox=dict(
                                    facecolor=color, alpha=0.7, edgecolor="none", pad=1
                                ),
                            )
    ...
    if mode in ["polygon", "both"]:
        title += (
            f", showing {drawn_polygons}/{min(len(annotations), max_polygons)} polygons"
        )
...
Examples:
  ...
  python vis_tools/visualize_lvis.py --save --output_dir vis_tools/output
    ...
    parser.add_argument(
        "--save", action="store_true", help="Save images instead of displaying"
    )
    ...
    sample_ids = random.sample(
        available_with_anns, min(args.num_samples, len(available_with_anns))
    )
    ...
    for idx, img_id in enumerate(sample_ids, 1):
        ...
        file_name = (
            img_info.get("coco_file_name")
            or img_info.get("file_name")
            or img_info.get("coco_url", "").split("/")[-1]
        )
