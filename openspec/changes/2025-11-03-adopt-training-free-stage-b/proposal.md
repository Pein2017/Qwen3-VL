...
- Replace the current Stage-B codepath with a **training-free, LLM-centred loop** modelled after Youtu-Agent’s training-free GRPO pattern but hardened for noisy Stage-A summaries and GT labels. The frozen checkpoint stays fixed; all improvement comes from repeatedly sampling candidates, judging against ground-truth labels, and letting an LLM synthesise guidance updates (our “optimizer step”). Legacy GRPO scaffolding is retired or shimmed so only one Stage-B implementation remains.
  ...
  4. **Deterministic Signals + Trust Scoring**: compute `label_match`, `self_consistency`, `candidate_agreement`, `confidence`, and `label_trust` while normalising verdict strings (`"pass"`/`"fail"`, `"通过"`/`"不通过"`). Signals log warnings on missing metrics but never misclassify due to localisation.
  5. **Three-Stage Reflection (Youtu-Agent style)**: after each batch, run (a) a **SampleSummarizer** that emits per-record `summary` and `critique` strings, (b) a **CritiqueScreener** that checks eligibility using the selected candidate plus an “all wrong” override, and (c) a **GuidanceConsolidator** that turns critiques into structured `{action, summary, critique, operations[...]}` JSON. Reflection calls must return syntactically valid JSON; stop-string truncation is fatal. Applied operations merge into guidance, capture provenance, and maintain at least one experience entry.
  6. **Selection & Export**: select final verdicts with the current experiences, prioritising GT-aligned candidates. Log warnings for problematic tickets but continue processing. Export trajectories, selections, summaries/critiques, and reflection logs so downstream QC can identify tickets that may need review.
...
- Run directory manifests:
  - `stats.json`, `epoch_{k}/shuffled_data.jsonl`, and `step_{n}/rollout.jsonl` all use deterministic keys so interrupted runs resume without recomputation. Missing manifests invalidate the run.
- Trajectory log (`output_post/stage_b/<mission>/<run>/step_{n}/trajectories.jsonl`):
  - `group_id`, `mission`, `candidate_index`, `decode` ({temperature, top_p, max_new_tokens, seed}), `response_text`, `guidance_step`, `created_at`, `verdict`, `reason`, `confidence`, `signals` ({label_match: bool, candidate_agreement: bool, self_consistency: float, confidence: float, label_trust: float}), `warning: bool`.
- Selection export (`selections.jsonl`/Parquet):
  - `group_id`, `mission`, `verdict`, `reason`, `confidence`, `label_match`, `label_trust`, `selected_candidate`, `guidance_step`, `reflection_change` (nullable reflection id), `warning: bool`.
- Reflection log (`reflection.jsonl`):
  - `reflection_id`, `mission`, `proposal` ({action, summary, critique, operations[{op, key, text, rationale, evidence}], evidence_group_ids, uncertainty_note}), `eligible: bool`, `applied: bool`, `guidance_step_before`, `guidance_step_after`, `warnings_logged: [group_id]`.
...
- `signals: {weights?: {label_match: float, self_consistency: float, candidate_agreement: float, confidence: float, label_trust: float}}` configures deterministic metrics and trust scoring (no separate verifier plugins; rely solely on LLM).
  - `reflection: {prompt_path: str, batch_size: int, apply_if_delta: float, allow_uncertain: bool}` for simplified direct application (no replay validation). Uses same checkpoint as Stage-B model, passed directly to `ReflectionEngine` for in-process model reuse.
...
3. **Signal Extraction**: `compute_signals` records deterministic metrics (`label_match`, `self_consistency`, `candidate_agreement`, `confidence`, `label_trust`) using LLM-based verification; groups with missing metrics log warnings and continue processing.
4. **Reflect**: `reflect_on_batch` packages winners/losers + signals + trust summaries, prompts an LLM (using the same checkpoint as Stage-B) to return an incremental edit payload (`{action: "refine" | "noop", summary?: str, critique?: str, operations: [{op: "upsert"|"remove", key?: str, text?: str, rationale?: str, evidence: [...] }], evidence_group_ids: [...] , uncertainty_note?: str}`), and evaluates it against holdout uplift before committing. The reflection engine reuses the rollout model/tokenizer, skips "trivial" bundles whose candidates agree, and only stages operations when they pass format checks. Operations merge into the existing experiences map (no wholesale replacement) and can both update existing keys and remove obsolete ones. A preview guidance map is built to compute post-uplift metrics; proposals that fail the `apply_if_delta` threshold or carry `uncertainty_note` when `allow_uncertain=false` are logged but not applied.
6. **Select**: `select_for_group` prioritises GT-aligned candidates while logging warnings for problematic tickets and stamping guidance step + reflection id onto each record.
7. **Export**: `export_trajectories`/`export_selections` capture outcomes with warning flags so operators can identify tickets that may need review.
...
- Require syntactically valid JSON reflection payloads; remove heuristic text fallback and adjust stop strings to prevent truncation.
- Introduce the Youtu-Agent style summary + critique pipeline and persist per-sample artifacts.
- Expand exports/docs/configs to cover the summary/critique fields and new validation hooks.
