...
### Persistence
- Fix snapshot ordering in `GuidanceRepository._write()`: write temp → atomic rename → copy snapshot of the new live file → prune. Add microsecond timestamped filenames and error-injection test. (Backlog / not in this change)
...
- Regression: atomic snapshot rotation with simulated failure; JSON parse failures remain fatal with `debug_info`. (Backlog / not in this change)
...
- **Add regression tests**: Cover verdict normalisation, guidance non-empty enforcement, atomic snapshot rotation, JSON parse failures, and summary/critique export wiring. (Backlog / not in this change)
...
### Remaining Items (Backlog / not in this change)
