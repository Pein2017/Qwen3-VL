...
- Packing needs an explicit domain-aware grouping key so source/target samples never mix inside a packed sequence.
- Offline fused JSONLs must carry the same provenance metadata as the online loader to keep downstream packers and debuggers aligned.
...
- Add a domain-aware packing/provenance requirement so records always include `_fusion_domain`/`_fusion_source` and packers group by domain.
...
- Minor contract change: fused JSONL outputs must include fusion metadata; packers must avoid cross-domain packing.
