...
| `stage_a_infer.sh` | Stage‑1 basic object recognition; emits per-image summaries JSONL. | `docs/INFERENCE_AND_STAGEA.md`, `docs/STAGE_A_STAGE_B.md` |
| `stage_b_run.sh` | Stage‑2 verdict loop (ingest → rollout → critic/selection → reflection) returning `pass|fail` per ticket. | `docs/STAGE_B_RUNTIME.md`, `docs/STAGE_A_STAGE_B.md` |
| `merge_stage2_lora.sh` | Merges Stage‑2 LoRA checkpoints for deployment. | `docs/INFERENCE_AND_STAGEA.md` |
