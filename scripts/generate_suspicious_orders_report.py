...
def main():
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        for result in analysis_data:
            ...
            f.write(f"  可疑比例: {suspicious/duplicate*100:.1f}%\n" if duplicate > 0 else "  可疑比例: N/A\n")
            f.write("\n")
        ...
        for mission, orders in suspicious_data['suspicious_orders_by_mission'].items():
            ...
            # 按审核不通过图片数排序，显示前20个
            sorted_orders = sorted(orders, key=lambda x: x['failed_images'], reverse=True)
            f.write("前20个最可疑的工单（按审核不通过图片数排序）：\n")
            for i, order in enumerate(sorted_orders[:20], 1):
                f.write(f"  {i}. {order['order_name']}: "
