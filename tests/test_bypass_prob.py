...
def test_bypass_prob_zero():
    ...
    preprocessor = AugmentationPreprocessor(
        augmenter=augmenter, rng=random.Random(42), bypass_prob=0.0
    )
...
def test_bypass_prob_one():
    ...
    preprocessor = AugmentationPreprocessor(
        augmenter=augmenter, rng=random.Random(42), bypass_prob=1.0
    )
...
def test_bypass_prob_intermediate():
    ...
    print(
        f"✓ bypass_prob=0.1 distribution: {bypass_count}/1000 bypassed (~{bypass_count / 10:.1f}%)"
    )
