...
from src.datasets.geometry import is_clockwise
...
def _poly_starts_at_top_left(poly: list[float]) -> bool:
    if not poly or len(poly) % 2 != 0:
        return True
...
def test_rotate_then_resize_bbox_to_poly_and_clip():
    ...
    cfg = {
        "ops": [
            {"name": "rotate", "params": {"max_deg": 10.0, "prob": 1.0}},
            {
                ...
                "params": {"lo": 0.9, "hi": 0.9, "align_multiple": None, "prob": 1.0},
            },
        ]
    }
...
def test_axis_aligned_hflip_then_resize_keeps_bbox():
    ...
    cfg = {
        "ops": [
            ...
            {
                "name": "resize_by_scale",
                "params": {"lo": 2.0, "hi": 2.0, "align_multiple": None, "prob": 1.0},
            },
        ]
    }
    compose: Compose = build_compose_from_config(cfg)
    ...
    assert (w, h) == (128, 64)
    g = out_geoms[0]
    assert "bbox_2d" in g and "poly" not in g
    x1, y1, x2, y2 = g["bbox_2d"]
    assert 0 <= x1 < x2 <= w - 1
    assert 0 <= y1 < y2 <= h - 1


def test_hflip_canonicalizes_poly_start_vertex_and_clockwise() -> None:
    """Regression: affine transforms must keep polys canonicalized for ordering + prompts."""
    rng = random.Random(123)
    cfg = {"ops": [{"name": "hflip", "params": {"prob": 1.0}}]}
    compose: Compose = build_compose_from_config(cfg)
    imgs = [_blank(64, 32)]
    # Start at top-left in the input; after hflip, the first vertex would become top-right
    # if we preserved the vertex list without rotating. We require top-left start.
    geoms = [{"poly": [10, 10, 30, 10, 30, 20, 10, 20]}]

    _, out_geoms = compose.apply(imgs, geoms, width=64, height=32, rng=rng)
    g = out_geoms[0]
    assert "poly" in g
    poly = g["poly"]
    assert is_clockwise(poly)
    assert _poly_starts_at_top_left(poly)
...
def test_rotate_with_expansion_and_32_alignment():
    """Rotation with canvas expansion: no cropping, dims are multiples of 32, polygons align."""
    rng = random.Random(42)
    cfg = {
        "ops": [
    ...
    # Polygon promoted from bbox due to rotation
    assert "poly" in out_geoms[0]
    q = out_geoms[0]["poly"]
    assert len(q) == 8
    # All polygon points within new bounds
    xs, ys = q[0::2], q[1::2]
    assert min(xs) >= 0 and max(xs) <= w - 1
    assert min(ys) >= 0 and max(ys) <= h - 1
...
def test_mixed_affines_with_expansion():
    ...
    # Check bbox promoted to poly, poly still poly, line still line
    assert "poly" in out_geoms[0]  # bbox → poly
    assert "poly" in out_geoms[1]
...
def test_pixel_limit_enforcement():
    ...
    cfg = {
        "ops": [
            {"name": "rotate", "params": {"max_deg": 45.0, "prob": 1.0}},
            {
                ...
                "params": {"multiple": 32, "max_pixels": max_pixels},
            },
        ]
    }
    compose: Compose = build_compose_from_config(cfg)
    ...
    assert "poly" in out_geoms[0]  # bbox promoted to poly due to rotation
    q = out_geoms[0]["poly"]
    assert len(q) == 8
...
def test_pixel_limit_identity_alignment_scaling():
    ...
    imgs = [
        _blank(80, 120)
    ]  # Alignment to 32 would produce 96×128 (12288 px) without scaling
    ...
    assert w % 32 == 0 and h % 32 == 0
    geom = out_geoms[0]
    coords = geom.get("poly") or geom.get("bbox_2d")
    xs = coords[0::2]
    ys = coords[1::2]
    assert min(xs) >= 0 and max(xs) <= w - 1
    assert min(ys) >= 0 and max(ys) <= h - 1
