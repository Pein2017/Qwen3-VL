...
def test_hflip_bbox_and_line_pixel_space():
    ...
    imgs_bytes, new_geoms = apply_augmentations_v2([img], geoms, pipe, rng=Random(0))
...
def test_rotate_quad_preserves_type_and_bounds():
    ...
    _, new_geoms = apply_augmentations_v2([img], geoms, pipe, rng=Random(123))
    q = new_geoms[0]["quad"]
...
def test_scale_bbox_non_degenerate_or_fallback():
    ...
    _, new_geoms = apply_augmentations_v2([img], geoms, pipe, rng=Random(7))
    bb = new_geoms[0]["bbox_2d"]
...
def test_color_jitter_geometry_unchanged():
    ...
    _, new_geoms = apply_augmentations_v2([img], geoms, pipe, rng=Random(5))
